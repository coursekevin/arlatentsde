...
def poly_transform(
    X: Tensor, degree: int, n_out_full: int, include_bias: bool
) -> Tensor:
    """Compute polynomial features taken directly from
    sklearn:https://github.com/scikit-learn/scikit-learn/blob/f3f51f9b6/sklearn/preprocessing/_polynomial.py#L30"""
    *n_samples, n_features = X.shape
    XP = torch.empty(*n_samples, n_out_full, dtype=X.dtype, device=X.device)
    ...
    for _ in range(2, degree + 1):
        ...
        for feature_idx in range(n_features):
            ...
            if next_col <= current_col:
                break
            XP[..., current_col:next_col] = (
                XP[..., start:end].clone().mul(X[..., feature_idx : feature_idx + 1])
            )
...
class SparsePolynomialFeatures(SparseFeaturesLibrary):
    ...
    def update_basis(self, sparse_index: Tensor) -> None:
        ...
        raise NotImplementedError(
            "Method for computing forward using sklearn implementation does not support sparse indexing of features"
        )
        # super().update_basis(sparse_index)
        # self.sparse_index = sparse_index
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Returns:
            Tensor: sparse dictionary evaluated at input (..., num_features[sparse_index])
        """
        features = poly_transform(
            x,
            degree=self.degree,
            n_out_full=self.num_features,
...
class SparseRandFourierFeatures(SparseFeaturesLibrary):
    """
    Abstract base class for making sparse feature libraries
    """

    def __init__(self, num_inputs: int, num_features: int) -> None:
        super().__init__()
        self.num_inputs = num_inputs
        self.num_features = num_features + num_inputs
        self.w = nn.Parameter(torch.randn(num_inputs, num_features))
        self.b = nn.Parameter(torch.rand(num_features) * math.pi * 2)

    def update_basis(self, sparse_index: Tensor) -> None:
        raise NotImplementedError("This method is out of date with current workflow")

    def forward(self, x: Tensor) -> Tensor:
        fourier = torch.cos(x.matmul(self.w).add(self.b)).mul(
            math.sqrt(2 / self.num_features)
        )
    ...
    @property
    def feature_names(self) -> List[str]:
        features = ["x_{}".format(i) for i in range(self.num_inputs)]
        features += [
            f"cos(w_{j}^T x + b_{j})"
            for j, (w_i, b_i) in enumerate(zip(self.w.T, self.b))
        ]
        return features
...
class SparsePolynomialNeighbour1D(SparsePolynomialFeatures):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        x_in = torch.stack(
            [x.roll(i, dims=-1) for i in self.inds],
            dim=-1,
        )
        return super().forward(x_in)
...
class SparseGLMGaussianLikelihood(nn.Module):
    def __init__(
        ...
        self.invsoftplus_sigma = Parameter(
            inverse_softplus(
                torch.tensor(
                    [
                        noise,
